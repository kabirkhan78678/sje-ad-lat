...
import type { AboutPageCollectionItem } from '@/modules/catalog/types/aboutPage';
import type { FieldConfig } from '@/types/resources';

type AboutPageCollectionManagerProps<TFormValues extends Record<string, unknown>> = {
  title: string;
  description: string;
  entityLabel: string;
  addLabel: string;
  ...
  fields: FieldConfig[];
  schema: ZodTypeAny;
  items: AboutPageCollectionItem[];
  defaultValues: TFormValues;
  isLoading?: boolean;
  isSaving?: boolean;
  onCreate: (values: TFormValues) => void;
  onUpdate: (item: AboutPageCollectionItem, values: TFormValues) => void;
  onDelete: (item: AboutPageCollectionItem) => void;
  onToggle: (item: AboutPageCollectionItem) => void;
  onReorder: (item: AboutPageCollectionItem, direction: 'up' | 'down') => void;
};

const sortItems = (items: AboutPageCollectionItem[]) =>
...
export const AboutPageCollectionManager = <TFormValues extends Record<string, unknown>>({
  title,
  description,
  entityLabel,
  addLabel,
  ...
  onToggle,
  onReorder,
}: AboutPageCollectionManagerProps<TFormValues>) => {
  const [activeItem, setActiveItem] = useState<AboutPageCollectionItem | null>(null);
  const [itemPendingDelete, setItemPendingDelete] = useState<AboutPageCollectionItem | null>(null);
  ...
  const openEdit = (item: AboutPageCollectionItem) => {
    setActiveItem(item);
          ...
          <DataTable
            columns={[
              ...previewFields.map((field) => ({
                key: field.name,
                label: field.label,
                render: (item: AboutPageCollectionItem) => renderPreviewValue(item[field.name], field),
              })),
              { key: 'display_order', label: 'Order' },
              {
                key: 'is_active',
                label: 'Status',
                render: (item: AboutPageCollectionItem) => (
