...
const extractUploadFile = (value: unknown): File | Blob | null => {
  ...
  if (Array.isArray(value)) {
    ...
    return firstValue instanceof File || firstValue instanceof Blob
      ? firstValue
      : null;
  }
...
const buildClientLogoPayload = (values: Record<string, any>) => {
  const payload = new FormData();
  const uploadFile = extractUploadFile(values.logo);

  payload.append("name", String(values.name ?? "").trim());
  payload.append("website", String(values.website ?? "").trim());
  payload.append("display_order", String(values.display_order ?? ""));

  if (uploadFile) {
    payload.append("logo", uploadFile);
  }
...
const heroFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Trusted Process Engineering",
  },
  {
    name: "title",
    label: "Title",
  ...
  },
  {
    ...
    rows: 5,
    placeholder:
      "Summarize the business promise for visitors in a concise, confident voice.",
    colSpan: 2,
  },
  {
    ...
    type: "file",
    description:
      "Upload an image or video file instead of pasting a hosted URL.",
    accept: "image/*,video/*",
    colSpan: 2,
  },
  {
...
export const heroConfig: SingletonResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "home-hero",
  title: "Hero Section",
  description:
    "Control the homepage hero message, media, and top-level call-to-action links.",
  fields: heroFields,
  sections: [
    {
      title: "Messaging",
      description:
        "Set the headline and supporting narrative that anchors the homepage.",
    ...
    },
    {
      ...
      description:
        "Highlight important quick-jump destinations beneath the hero.",
      ...
      <div className="space-y-5 bg-[radial-gradient(circle_at_top_left,rgba(31,159,120,0.35),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(249,143,7,0.26),transparent_24%)] px-6 py-7">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm leading-7 text-slate-200">
            {values.subtitle ||
              "Supporting copy will appear here as content managers update the hero section."}
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <Button>{values.primary_button_text || "Primary CTA"}</Button>
          <Button variant="outline">
            {values.secondary_button_text || "Secondary CTA"}
          </Button>
        </div>
        <div className="flex flex-wrap gap-2 pt-2">
          {(
            values.ribbon_links as
...
const statsFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "projects_delivered",
  },
  {
    name: "stat_value",
    label: "Stat Value",
...
export const homeStatsConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "home-stats",
  title: "Home Stats",
  description:
    "Manage key stats shown on the homepage and keep their order consistent.",
  entityLabel: "Stat",
  fields: statsFields,
  ...
  emptyState: {
    ...
    description:
      "Add business stats to showcase milestones, capacities, or reach on the homepage.",
  },
};
...
const servicesFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Turnkey Engineering Services",
  },
  {
    name: "description",
    label: "Description",
    ...
    rows: 4,
    placeholder:
      "Describe this service group in a concise and benefit-driven way.",
    colSpan: 2,
  },
...
export const servicesConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "home-services",
  title: "Services",
  description:
    "Publish core service cards, service item lists, and CTA links used on the homepage.",
  entityLabel: "Service",
  ...
  emptyState: {
    ...
    description:
      "Add service content to organize the homepage offering overview.",
  },
};
...
const whyChooseUsFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Hands-on process expertise",
  },
  {
    name: "description",
    label: "Description",
...
export const whyChooseUsConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "why-choose-us",
  title: "Why Choose Us",
  description:
    "Curate the differentiators that help explain SJE’s operational strengths.",
  ...
  emptyState: {
    ...
    description:
      "Add reasons customers should trust SJE across execution and service quality.",
  },
};
...
const machineryFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Automatic Bottle Filling Line",
  },
  {
    name: "description",
    label: "Description",
  ...
  { name: "badge", label: "Badge", type: "text", placeholder: "High Output" },
  {
    ...
    type: "text",
    required: true,
    placeholder: "water-line",
  },
  {
    name: "capacity_label",
    label: "Capacity Label",
    type: "text",
    placeholder: "12,000 BPH",
  },
  {
...
export const machineryConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "machinery",
  title: "Machinery",
  description:
    "Manage machinery cards, filters, and output ranges shown across the homepage.",
  entityLabel: "Machine",
  ...
  emptyState: {
    ...
    description:
      "Add machinery items with categories, sections, and output ranges.",
  },
};
...
const turnkeyFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Site discovery and planning",
  },
  {
    name: "description",
    label: "Description",
...
export const turnkeyStepsConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "turnkey-steps",
  title: "Turnkey Steps",
  description:
    "Sequence the stages used to explain SJE’s turnkey project process.",
  ...
  columns: [
    ...
    { key: "icon", label: "Icon" },
    {
      key: "items",
      label: "Items",
      render: (record) => `${record.items?.length ?? 0} items`,
    },
    { key: "display_order", label: "Order" },
  ],
  searchKeys: ["title", "description"],
  ...
  emptyState: {
    ...
    description:
      "Add the process stages that explain the project journey from start to handover.",
  },
};
...
const productionConfigurationFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Mid-volume production",
  },
  {
    name: "bph_min",
    label: "BPH Min",
...
export const productionConfigurationsConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  ...
  description:
    "Map production ranges to recommended machinery bundles and deployment summaries.",
...
const labEquipmentFields: FieldConfig[] = [
  {
    ...
    type: "text",
    required: true,
    placeholder: "Bottle burst tester",
  },
  {
    name: "category",
    label: "Category",
...
export const labEquipmentConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "lab-equipment",
  title: "Lab Equipment",
  description:
    "Manage laboratory and QA equipment entries, specifications, and featured badges.",
  ...
  columns: [
    ...
    { key: "manufacturer", label: "Manufacturer" },
    {
      ...
      label: "Featured",
      render: (record) => (
        <Badge tone={record.is_featured ? "success" : "neutral"}>
  ...
  emptyState: {
    ...
    description:
      "Add lab tools, specifications, and featured items for the homepage.",
  },
...
const clientLogosFields: FieldConfig[] = [
  ...
  },
  {
    ...
    description: "Upload a logo file instead of entering a logo URL.",
    colSpan: 2,
  },
  {
    name: "website",
...
export const clientLogosConfig: CrudResourceConfig<
  Record<string, any>,
  Record<string, any>
> = {
  id: "client-logos",
  title: "Client Logos",
  description:
    "Manage client or partner logos, destination links, and their display order.",
  ...
  transformPayload: (values) => buildClientLogoPayload(values),
  columns: [
    { key: "name", label: "Name" },
    {
      key: "logo",
      label: "Logo",
      render: (record) => {
        const logoUrl = resolveMediaUrl(record.logo_url ?? record.logo);

        return logoUrl ? (
          <img
            alt={String(record.name ?? "Client logo")}
            className="h-12 w-24 rounded-lg border border-slate-200 bg-white object-contain p-2"
            src={logoUrl}
          />
        ) : (
          "No logo"
  ...
  emptyState: {
    ...
    description:
      "Add logos to display customer or partner trust signals on the homepage.",
  },
};
